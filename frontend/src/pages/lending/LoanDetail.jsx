...
const NEXT = {
  ...
  underwriting: [{ to: "approved", label: "Approve & Disburse (B2C)", cls: "btn-primary" },
                 { to: "rejected", label: "Reject", cls: "btn-ghost !text-red-600" }],
...
export default function LoanDetail() {
  ...
  const transition = async (to) => {
    ...
    try {
      ...
      setMsg(to === "approved" ? "Loan approved — funds disbursed via M-Pesa B2C and SMS sent." : `Loan moved to ${to}.`);
      load();
  ...
  const actions = (NEXT[loan.status] || []).map((a) => (
